...
if (Poi.reply_link_version == 'new') {
	$('body').on('click', '.comment-reply-link', function() {
		addComment.moveForm("comment-" + $(this).attr('data-commentid'), $(this).attr('data-commentid'), "respond", $(this)
			.attr('data-postid'));
		return false;
	});
}
				...
				success: function(res) {
					...
					if (res.status == 200) {
						...
						$('#upload-img-show').append(
							'<img class="lazyload upload-image-preview" src="https://cdn.jsdelivr.net/gh/mirai-mamori/web-img/loadimg/inload.svg" data-src="' +
...
function tableOfContentScroll(flag) {
	if (document.body.clientWidth <= 1200) {
		...
		if (flag) {
			var id = 1,
				heading_fix = mashiro_option.entry_content_theme == "sakura" ? $("article").hasClass("type-post") ? $("div").hasClass(
					"pattern-attachment-img") ? -75 : 200 : 375 : window.innerHeight / 2;
		...
		$('body').on('click', '#bangumi-pagination a', function() {
			...
			xhr.onreadystatechange = function() {
				if (xhr.readyState == 4 && xhr.status == 200) {
					...
					$("#bangumi-pagination a").removeClass("loading").html(
				...
				if (mashiro_option.live_search) {
					...
					function search_result(keyword, link, fa, title, iconfont, comments, text) {
						if (keyword) {
							...
							title = title.replace(s[s.length - 1], '<mark class="search-keyword"> ' + s[s.length - 1].toUpperCase() +
								' </mark>');
							text = b < 60 ? text.slice(0, 80) : text.slice(b - 30, b + 30);
							text = text.replace(s[s.length - 1], '<mark class="search-keyword"> ' + s[s.length - 1].toUpperCase() +
								' </mark>');
						}
						return '<div class="ins-selectable ins-search-item" href="' + link + '"><header><i class="fa fa-' + fa +
							'" aria-hidden="true"></i>' + title + '<i class="iconfont icon-' + iconfont + '"> ' + comments + '</i>' +
							'</header><p class="ins-search-preview">' + text + '</p></div>';
					}
					...
					function query(B, A, z) {
						...
						w && (y = y + G + "文章" + E + w + D), u && (y = y + G + "页面" + E + u + D), r && (y = y + G + "分类" + E + r + D),
							p && (y = y + G + "标签" + E + p + D), F && (y = y + G + "评论" + E + F + D), s = addComment.I("PostlistBox"), s.innerHTML =
							y
					}
				}
			});
